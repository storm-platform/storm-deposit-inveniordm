...
# TEMP
from storm_deposit_inveniordm.config import STORM_DEPOSIT_INVENIORDM_SERVER_URL
...
def service_task(
    ...
    **kwargs,
    ...
    project_metadata = render_template(
        "metadata.json", config.TEMPLATE_PATH, project=project
    )
    ...
    invenio_service = InvenioRDM(STORM_DEPOSIT_INVENIORDM_SERVER_URL, access_token)

    created_draft = RecordDraft(project_metadata)
    created_draft = invenio_service.records.draft().create(created_draft)

    invenio_service.records.files(created_draft).upload_files(
